...
- **Home** — Greenhouse, Basement, Closet, Desktop, or Workshop room (theme in localStorage), status shelves (In season / Resting / Proud shelf / Archive), create form, progress chooser when tending, **Hyperfocus** (one-hobby focus view from the tend bench), gentle in-app cadence nudges, optional **progress photos** on the tend bench
...
- Progress, nested items, and nudge timing are stored in localStorage on your device. Optional progress photo blobs live in IndexedDB (metadata keys in localStorage).
